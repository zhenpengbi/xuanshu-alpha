...
# 真实可穿透持仓基金（持仓穿透分析对象）
# P0: 008585(AI主题), 515790(光伏ETF)
# P1 扩展: 001631(AI主题联接A), 014880(机器人ETF联接C), 014658, 015897
# ⚠️ 014658/015897 官方名与 portfolio.json 标注不符（已在 value_compass.json 中 name_mismatch_warning 记录）
PILOT_FUNDS = ["008585", "515790", "001631", "014880", "014658", "015897"]
...
CODE_CIRCLE = {
    ...
    # ── P1 新增 ──────────────────────────────────────────────────
    # 001631 持仓（实际是食品饮料基金，名称与 portfolio 不符）
    "600519": "in",          # 贵州茅台(白酒)
    "000858": "in",          # 五粮液(白酒)
    "000568": "in",          # 泸州老窖(白酒)
    "600809": "in",          # 山西汾酒(白酒)
    ...
    # 014658 中欧融享增益 重仓（偏债混合，实际非高端制造）
    "601665": "unknown",     # 齐鲁银行(银行，不在能力圈框架内)
    "601166": "unknown",     # 兴业银行(银行)
    "300836": "unknown",     # 宏桥控股(铝/有色，港股通)
    "688219": "out",         # 君实生物(创新药/生物科技，超出能力圈)
    "002625": "unknown",     # 光启技术(高新技术)
    # 015897 细分化工产业ETF 重仓（生命科学/创新药，超出能力圈）
    "688783": "out",         # 西安奕材(新材料)
    "688765": "out",         # 禾元生物(生物技术)
    "688759": "out",         # 必贝特(创新药)
    "301683": "out",         # 慧谷新材(新材料)
    "301682": "out",         # 宏明电子(电子元器件)
}
