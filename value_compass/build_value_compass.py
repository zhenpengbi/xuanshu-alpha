...
# 穿透标的：从 portfolio.json 动态读取，禁止硬编码旧代码
# 规则：assetType ∈ {oscillation, active} 且 category ∉ {黄金, 有色金属, 货币基金}
# 当前覆盖：012885(光伏), 011840(AI), 014881(机器人), 015790(永赢高端装备), 025647(平安高端装备)
def _load_pilot_funds():
    try:
        import json as _j
        with open(PORTFOLIO_PATH, encoding="utf-8") as _f:
            pf = _j.load(_f)
        skip_cat  = {"黄金", "有色金属", "货币基金"}
        skip_type = {"trend", "cash"}
        funds = [
            h["code"] for h in pf.get("holdings", [])
            if h.get("assetType") not in skip_type
            and h.get("category") not in skip_cat
        ]
        return funds if funds else ["012885", "011840", "014881", "015790", "025647"]
    except Exception as e:
        print(f"  ⚠️ portfolio.json 读取失败，使用内置列表: {e}")
        return ["012885", "011840", "014881", "015790", "025647"]

PILOT_FUNDS = _load_pilot_funds()
...
CODE_CIRCLE = {
    ...
    # ── P1 校准版（使用 portfolio.json 最终正确代码）────────────────
    # 014881 天弘机器人ETF 重仓（与 014880 相同标的体系）
    "002008": "borderline",  # 大族激光(激光设备)
    "300024": "borderline",  # 机器人(工业机器人)
    "002031": "borderline",  # 巨轮智能(工业自动化)
    "000967": "borderline",  # 盈峰环境(环保/工业设备)
    "002747": "borderline",  # 埃斯顿(工业机器人)
    # 015790 永赢高端装备智选 重仓（航天/国防电子为主，超出巴菲特能力圈）
    "600879": "out",         # 航天电子(国防电子/军工，高技术壁垒)
    "600118": "out",         # 中国卫星(航天/军工)
    "300342": "out",         # 天银机电(军工电子)
    "688375": "out",         # 国博电子(国防电子/GaN)
    "601698": "out",         # 中国卫通(卫星通信/航天央企)
    "300136": "borderline",  # 信维通信(通信天线/器件，制造业)
    "688568": "out",         # 中科星图(卫星遥感/AI，超出能力圈)
    "300102": "out",         # 乾照光电(化合物半导体/LED)
    "600498": "borderline",  # 烽火通信(光通信设备)
    "301005": "borderline",  # 超捷股份(精密零部件/工业自动化)
    # 025647 平安高端装备 重仓（航天/半导体/雷达为主，超出能力圈）
    "002149": "borderline",  # 西部材料(稀有金属/新材料)
    "688387": "out",         # 信科移动(5G通信技术/芯片)
    "688270": "out",         # 臻镭科技(雷达射频芯片)
    "001400": "borderline",  # 江顺科技(工业零部件)
    "688378": "out",         # 奥来德(OLED有机材料/半导体)
    "688385": "out",         # 复旦微电(FPGA/国产芯片)
    "688818": "out",         # 电科蓝天(国防电子/雷达)
    # 012885 光伏ETF联接（持仓与名称不一致，以实际拉取数据为准）
    "688795": "out",         # 摩尔线程(国产GPU/算力芯片)
    "001280": "borderline",  # 中国铀业(核能/大宗商品)
    "301638": "borderline",  # 南网数字(电力数字化/工业)
    "688802": "out",         # 沐曦股份(AI算力芯片)
    "001386": "unknown",     # 马可波罗(家居陶瓷)
}
